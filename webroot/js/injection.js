$(document).ready(function(){
    ...
    $( ".mobile" ).keypress(function(e) {
        ...
        if (key >= 31 && key <= 46 || key >= 58 && key <= 126) {
            ...
            $('#'+id).attr('placeholder', 'Invalid value, Only character, comma and small opening & closing braces accepted !.');
            e.preventDefault();
        }else if(value.chatAt(0) < 6){
            $('#'+id).val('');
            $('#'+id).css('border-color', 'red');
            $('#'+id).attr('placeholder', 'Mobile number should start 6 or greater than 6 !.');
        }else if((valLength < 10) || ((valLength > 10))){
            $('#'+id).val('');
            $('#'+id).css('border-color', 'red');
            $('#'+id).attr('placeholder', 'Mobile number should be 10 digit long only !.');
        }
    });
    /** Ended : For Mobile Number only */
